...
function success {
  ...
  then
    ...
    MESSAGE="testing success string"
  else
...
function failure {
  if [ "$TRAVIS_PULL_REQUEST" != "false" ]
  then
    MESSAGE="Travis [build no. $TRAVIS_BUILD_NUMBER](travis-ci.org/$TRAVIS_REPO_SLUG/builds/$TRAVIS_BUILD_ID) has failed. Pull request no. $TRAVIS_PULL_REQUEST can be found [here](https://github.com/${TRAVIS_REPO_SLUG}/pull/${TRAVIS_PULL_REQUEST})."
  else
